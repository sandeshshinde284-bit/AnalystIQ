...
class VertexAIService {
  ...
  constructor() {
    ...
    this.location = this.getEnvVar("VITE_VERTEX_AI_LOCATION", "us-central1");
    this.model = this.getEnvVar("VITE_VERTEX_AI_MODEL", "text-bison-001");
...
- Regulatory and compliance risks`;
  ...
  private getEnvVar(key: string, fallback = ""): string {
    try {
      ...
      console.warn(
        `Failed to access environment variable ${key}, using fallback`
      );
      return fallback;
    }
  }
