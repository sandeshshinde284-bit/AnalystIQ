//ANALYSISORCHESTRATOR.SERVICE.TS
import documentAIService from "./documentAI.service.js";
import cloudVisionService from "./cloudVision.service.js";
import geminiService from "./gemini.service.js";
import bigQueryService from "./bigquery.service.js";
import firebaseService from "./firebase.service.js";
import agentBuilderService from "./agentBuilder.service.js";
...
class AnalysisOrchestrator {
  ...
  private async processDocuments(
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      // Update progress
      const fileProgress = (i / files.length) * 40;
      await this.updateProgress(
        analysisId,
        {
          status: "processing",
          ...
          message: `Processing document: ${file.fieldname}`,
        },
        options.progressCallback
      );
      ...
      try {
        ...
        const docResult = await documentAIService.processDocument(
          ...
          (message: string, progress: number) => {
            ...
            this.updateProgress(
              ...
              {
                status: "processing",
                step: `processing-${file.originalname}`,
                progress: overallProgress,
        ...
        if (options.includeVisionAnalysis !== false) {
          try {
            ...
            console.warn(
              `Vision analysis failed for ${file.originalname}:`,
              error
            );
