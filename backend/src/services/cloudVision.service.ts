//cloudVision.service.ts

import { googleCloudConfig } from "../config/googleCloud.js";

// This interface represents the file object provided by an upload
// middleware like `multer`. It uses Node.js's `Buffer`.
...
class CloudVisionService {
  ...
  // ✅ Works with a Node.js Buffer, not a browser-specific ArrayBuffer
  ...
  // ✅ This method is now designed for a backend environment
  async analyzeDocumentFromFile(
    file: UploadedFile,
    documentType: string
  ): Promise<VisionAnalysisResult> {
    try {
      // No conversion needed! The upload middleware gives us the buffer.
      return this.analyzeDocument(file.buffer, documentType);
    } catch (error) {
  ...
  async detectChartsAndGraphs(imageBuffer: Buffer): Promise<ChartObject[]> {
    try {
  ...
  // ❌ REMOVED the fileToArrayBuffer method as it's no longer needed.
  ...
  async healthCheck(): Promise<boolean> {
    try {
      const testBuffer = Buffer.alloc(100); // Use Node.js Buffer
